...
$DisplayNameNew = Read-Host -Prompt "Specify a new policy name instead of [$($JSON_Convert.displayName)].  Suggestion: [$("$($JSON_Convert.displayName) - $(Get-Date -Format 'yyyyMMdd')")]"
If (-not([string]::IsNullOrEmpty($DisplayNameNew))) {
	$JSON_Convert.displayName = $DisplayNameNew
	$DisplayName = $JSON_Convert.displayName
}
