...
in the Console. The equivilent action in the gui to that if the "Unlimited when sending to this destination" radio button is NOT set, then the Rate limits are enabled. If it is set then Rate Limits are not enabled. 
...
.EXAMPLE
Get-DPRateLimits -SiteServer DSCCMXX01 -SiteCode LAB -Enable $true
Query for Distribution Points with Rate Limits enabled
.EXAMPLE
Get-DPRateLimits -SiteServer DSCCMXX01 -SiteCode LAB -Enable $false
...
[Boolean]$Enable=$True
	...
	if($Enable){
		$dp = gwmi -Computername $siteserver -namespace "root\sms\site_$sitecode" -query "select * from SMS_SCI_address where UnlimitedRateForAll = 'False'" 
	}else{
		$dp = gwmi -Computername $siteserver -namespace "root\sms\site_$sitecode" -query "select * from SMS_SCI_address where UnlimitedRateForAll = 'True'" 
	}
    return $dp
} 