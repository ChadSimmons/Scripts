...
#	========== Change Log History ==========
#	- 2019/05/31 by Chad.Simmons@CatapultSystems.com - minor updates... latest version
#	- 2019/05/15 by Chad.Simmons@CatapultSystems.com - added Restart parameter
...
[CmdletBinding()]
Param (
	[Parameter(Mandatory=$true)][ValidateNotNullorEmpty()][string]$ShareFinalPath, #$(Package/Task Sequence ID).$ComputerName
	[Parameter()][string]$ShareBasePath = 'Share\Folder\Subfolder\Logs', #if blank will ???
	[Parameter()][string]$LogServer = 'Server.Contoso.com', #if blank will detect ConfigMgr MP
	[Parameter()][string]$DeleteLocalArchive = 'false', #ConfigMgr 1806 only supports String and Int, not [switch].
	[Parameter()][string]$SuppressConsoleOutput = 'true', #ConfigMgr 1806 only supports String and Int, not [switch].  Disabling Console output to control the captured script output
	[Parameter()][string]$Restart = 'false', #ConfigMgr 1806 only supports String and Int, not [switch].  Disabling Console output to control the captured script output
	#[Parameter()][ValidateSet('true','false')][string]$DeleteLocalArchive = 'false', #ConfigMgr 1806 only supports String and Int, not [switch].
	#[Parameter()][ValidateSet('true','false')][string]$SuppressConsoleOutput = 'true', #ConfigMgr 1806 only supports String and Int, not [switch].  Disabling Console output to control the captured script output
	#[Parameter()][ValidateSet('true','false')][string]$Restart = 'false', #ConfigMgr 1806 only supports String and Int, not [switch].  Disabling Console output to control the captured script output
	[Parameter()][ValidateRange(5,600)][int]$RestartDelay = 30 #seconds to delay before restarting
)
...
Function Get-ScriptInfo {
	#.Synopsis
	#   Get the folder, name, etc. of the script file
	#.Notes
	...
	If ($psISE -and [string]::IsNullOrEmpty($script:ScriptPath)) {
		$script:ScriptFile = $psISE.CurrentFile.FullPath #this works in psISE and psISE functions
	} else {
		try {
			$script:ScriptFile = $((Get-Variable MyInvocation -Scope 1 -ErrorAction SilentlyContinue).Value).MyCommand.Source
		} catch {
	...
	Write-Verbose "Function Get-ScriptInfo: ScriptStartTime is $($script:ScriptStartTime)"
	Write-Verbose "Function Get-ScriptInfo: ScriptFile is $($script:ScriptFile)"
	Write-Verbose "Function Get-ScriptInfo: ScriptPath is $($script:ScriptPath)"
	Write-Verbose "Function Get-ScriptInfo: ScriptName is $($script:ScriptName)"
	Write-Verbose "Function Get-ScriptInfo: ScriptBaseName is $($script:ScriptBaseName)"
	#Get-Variable -Name 'script*' | ForEach-Object { Write-Verbose "Function Get-ScriptInfo: Variable [$($_.Name)] = [$($_.Value)]" }
}
...
Function Start-Script ($LogFile) {
	If ([string]::IsNullOrEmpty($script:ScriptFile)) { Get-ScriptInfo }
	If ([string]::IsNullOrEmpty($script:ScriptFile)) { Write-Error 'Get-ScriptInfo function must be called first'; Break }

	#If the LogFile is undefined set to \Windows\Logs\<ScriptName>log #<ScriptPath>\<ScriptName>.log
	If ([string]::IsNullOrEmpty($LogFile)) {
		#$script:LogFile = "$env:WinDir\Logs\$($script:ScriptBaseName).log"
		$script:LogFile = "$env:WinDir\Logs\$([System.IO.Path]::ChangeExtension($script:ScriptName, 'log'))"
    ...
    #$script:ScriptLocationAtStart = Get-Location
    #If ((Get-Location).Provider -ne 'FileSystem') { Push-Location -Path $env:SystemDrive }
	Write-LogMessage -Message "==================== SCRIPT START ====================" -Component $script:ScriptBaseName
	Write-LogMessage -Message "Started [$script:ScriptName] from [$script:ScriptPath] on $(Get-Date -Date $script:ScriptStartTime -Format 'F')" -Component $script:ScriptBaseName
	Write-LogMessage -Message "Script modified date is $(Get-Date -Date $((Get-Item -Path $script:ScriptFile).LastWriteTime) -Format 'yyyy/MM/dd HH:mm:ss')" -Component $script:ScriptBaseName
	Write-LogMessage -Message "Logging to file [$script:LogFile]" -Component $script:ScriptBaseName
...
Function Stop-Script ($ReturnCode) {
	...
	If ($Restart -eq 'true') {
		$ReturnCode = 3010
		Write-LogMessage -Message "Restart parameter enabled, changing Return Code to 3010 and spawning process shutdown.exe /r /f /t $RestartDelay"
		Start-Process -FilePath "$env:SystemRoot\System32\shutdown.exe" -ArgumentList "/r /f /t $RestartDelay"
	}
...
$ReturnCode = 0
$script:OutputArray = @()
##Transform DeleteLocalArchive to boolean defaulting to true
...
try {
	...
	try {
		...
		Write-LogMessage -Message "Error $($_.Exception.HResult) : $($_.Exception.Message)"
