...
echo ===============================================================================
echo .Synopsis
echo    Install/Upgrade Windows hardware drivers
echo .Description
echo    using PNPUtil.exe, install/update hardware drivers for Microsoft Windows 11 / 10 / 8.1 / 8 / 7
echo    If Drivers.wim exists, mount it before running PNPUtil.exe and unmount it afterwards
echo .Functionality
echo    Hardware Driver Upgrade
echo .Parameters
...
echo    2020/09/09 by Chad.Simmons@CatapultSystems.com - Added script header and exit code 3010 logic
...
PnPutil.exe /add-driver "%StageDir%\*.inf" /subdirs /install >> "%WinDir%\CCM\Logs\Update-Drivers.log"
SET rc=%errorlevel%
echo PNPUtil.exe completed with exit code %rc% >> "%WinDir%\CCM\Logs\Update-Drivers.log"
...
echo Script %~nx0 completed with exit code %rc% >> "%WinDir%\CCM\Logs\Update-Drivers.log"
EXIT /b %rc%